  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-hull-400">Overview of your user base</p>
      </div>
        ...
        {statCards.map((stat) => (
          <Card key={stat.label}>
            <div className="flex items-center">
              ...
              <div className="ml-4">
                <p className="text-sm text-hull-400">{stat.label}</p>
                <p className="text-2xl font-semibold text-hull-100">{stat.value}</p>
              </div>
            </div>
          </Card>
        ))}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <Card>
          <h2 className="text-lg font-semibold text-hull-100 mb-4">Users by Role</h2>
          <div className="space-y-4">
            {roleCards.map((role) => (
              <div key={role.label} className="flex items-center justify-between">
                <span className="text-hull-400">{role.label}</span>
                <div className="flex items-center">
                  <div className="w-32 h-2 bg-space-700 rounded-full mr-3">
                    <div
                      className="h-2 bg-primary-500 rounded-full"
                      style={{ width: `${total > 0 ? (role.value / total) * 100 : 0}%` }}
                    />
                  </div>
                  <span className="text-hull-100 font-medium w-8 text-right">{role.value}</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <Card>
          ...
          <div className="grid grid-cols-2 gap-4">
            <Link
              ...
              className="flex items-center p-4 rounded-lg border border-space-600 hover:border-primary-500 hover:bg-primary-500/10 transition-colors"
            >
              <svg className="w-8 h-8 text-primary-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M18 9v3m0 0v3m0-3h3m-3 0h-3m-2-5a4 4 0 11-8 0 4 4 0 018 0zM3 20a6 6 0 0112 0v1H3v-1z" />
              </svg>
              <span className="ml-3 font-medium text-hull-100">Add User</span>
            </Link>
            <Link
              href="/admin/users"
              className="flex items-center p-4 rounded-lg border border-space-600 hover:border-primary-500 hover:bg-primary-500/10 transition-colors"
            >
              ...
              <span className="ml-3 font-medium text-hull-100">View All Users</span>
            </Link>
          </div>
        </Card>
      </div>
